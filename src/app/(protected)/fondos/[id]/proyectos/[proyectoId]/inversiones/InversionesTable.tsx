...
export function InversionesTable({
  ...
  const filteredInversiones = useMemo(() => {
    ...
    return inversiones.filter((inv) => inv.inversionistaNombre.toLowerCase().includes(searchLower));
  }, [inversiones, search]);
  ...
  const handleRowClick = (inversion: InversionListItem) => {
    ...
    router.push(`/fondos/${fondoId}/inversiones/${inversion.id}`);
  };
      ...
      accessor: (inversion) => (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-foreground truncate font-medium">{inversion.inversionistaNombre}</p>
          </div>
        </div>
      ),
    },
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Participaciones de inversionistas en este proyecto
        </p>
      </div>
